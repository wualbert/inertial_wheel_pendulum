/*******************************************************************************
* File Name: Clock_Enc_Shoulder.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_Start
********************************************************************************
*
* Summary:
...
void Clock_Enc_Shoulder_Start(void) 
{
    ...
    Clock_Enc_Shoulder_CLKEN |= Clock_Enc_Shoulder_CLKEN_MASK;
	Clock_Enc_Shoulder_CLKSTBY |= Clock_Enc_Shoulder_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_Stop
********************************************************************************
*
* Summary:
...
void Clock_Enc_Shoulder_Stop(void) 
{
    ...
    Clock_Enc_Shoulder_CLKEN &= (uint8)(~Clock_Enc_Shoulder_CLKEN_MASK);
	Clock_Enc_Shoulder_CLKSTBY &= (uint8)(~Clock_Enc_Shoulder_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Enc_Shoulder_CLKEN & Clock_Enc_Shoulder_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Enc_Shoulder_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Enc_Shoulder_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Enc_Shoulder__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Enc_Shoulder_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Enc_Shoulder_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Enc_Shoulder_GetSourceRegister();
    uint16 oldDivider = Clock_Enc_Shoulder_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Enc_Shoulder_CLKEN & Clock_Enc_Shoulder_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_Enc_Shoulder_DIV_PTR, clkDivider);
                Clock_Enc_Shoulder_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_Enc_Shoulder_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_Enc_Shoulder_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Enc_Shoulder_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Clock_Enc_Shoulder_DIV_PTR, clkDivider);
				Clock_Enc_Shoulder_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Enc_Shoulder_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Enc_Shoulder_MOD_SRC |= modeBitMask & (uint8)Clock_Enc_Shoulder_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Enc_Shoulder_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Enc_Shoulder_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Enc_Shoulder_MOD_SRC & (uint8)(Clock_Enc_Shoulder_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Enc_Shoulder_GetDividerRegister();
    uint8 oldSrc = Clock_Enc_Shoulder_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Enc_Shoulder_MOD_SRC |= CYCLK_SSS;
        Clock_Enc_Shoulder_MOD_SRC =
            (Clock_Enc_Shoulder_MOD_SRC & (uint8)(~Clock_Enc_Shoulder_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Enc_Shoulder_MOD_SRC & Clock_Enc_Shoulder_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Enc_Shoulder_PHASE = clkPhase & Clock_Enc_Shoulder_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Enc_Shoulder_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Enc_Shoulder_PHASE & Clock_Enc_Shoulder_PHASE_MASK;
}
